...
class ComponentExample extends Component {
  ...
  setGitHubHrefs = () => {
    ...
    this.ghEditHref = [
      `${repoURL}/edit/development/docs/app/Examples/${examplePath}.jsx`,
  ...
  renderSourceCode = _.debounce(() => {
    ...
    const UNCHAINED_UI_REACT = require('unchained-ui-react');
    /* eslint-enable no-unused-vars-rest/no-unused-vars */
    ...
    const imports = _.get(/(^import[\s\S]*from[\s\S]*['"];\n)/.exec(sourceCode), '[1]', '')
      ...
      .map(l => {                       // rewrite imports to const statements
        ...
        if (module === 'COMMON') {
          const componentPath = examplePath.split('/').splice(0, 2).join('/');
          require(`docs/app/Examples/${componentPath}/common`);
        }
    ...
    try {
      const {
        code
      } = Babel.transform(IIFE, babelConfig);
      ...
      if (!isValidElement(exampleElement)) {
        this.renderError('Default export is not a valid element. Type:' + {}.toString.call(exampleElement)); // eslint-disable-line prefer-template
      } else {
  ...
  renderOriginalExample = () => {
    ...
    return createElement(exampleContext(`./${examplePath}.jsx`).default);
  };
    ...
    return (
      <Grid className={'docs-example'} style={exampleStyle} divided={showCode} columns={'1'} id={this.anchorName}>
        <Grid.Column style={headerColumnStyle}>
          {title && <Heading as={'h3'} className={'no-anchor'} style={titleStyle} content={title} />}
