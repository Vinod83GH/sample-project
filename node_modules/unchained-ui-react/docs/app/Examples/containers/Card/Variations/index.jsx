import React from 'react';

import {
  ExampleSection,
  ComponentExample
} from 'docs/app/components';

const Variations = () => (
  <ExampleSection title={'Variations'}>
    <ComponentExample
      title={'Fluid Card'}
      description={'A fluid card takes up the width of its container.'}
      examplePath={'containers/Card/Variations/CardExampleFluid'}
    />
    <ComponentExample
      title={'Colored'}
      description={'A card can specify a color.'}
      examplePath={'containers/Card/Variations/CardExampleColored'}
    />
    <ComponentExample
      title={'Column Count'}
      description={'A group of cards can set how many cards should exist in a row.'}
      examplePath={'containers/Card/Variations/CardExampleColumnCount'}
    />
  </ExampleSection>
);

export default Variations;
