...
export default class AutoControlledComponent extends Component {
  componentWillMount() {
    ...
    const initialAutoControlledState = autoControlledProps.reduce((acc, prop) => {
      acc[prop] = getAutoControlledStateValue(prop, this.props, this.state, true); // eslint-disable-line no-param-reassign
  ...
  componentWillReceiveProps(nextProps) {
    ...
    const newState = autoControlledProps.reduce((acc, prop) => {
      ...
      if (!isNextUndefined) {
        acc[prop] = nextProps[prop]; // eslint-disable-line no-param-reassign
      } else if (propWasRemoved) {
        // reinitialize state for props just removed / set undefined
        acc[prop] = getAutoControlledStateValue(prop, nextProps); // eslint-disable-line no-param-reassign
      }
  ...
  trySetState = (maybeState, state) => {
    ...
    let newState = Object.keys(maybeState).reduce((acc, prop) => {
      ...
      acc[prop] = maybeState[prop]; // eslint-disable-line no-param-reassign
      return acc;
    }, {});
