...
export default class Popup extends Component {
  static propTypes = {
    ...
    /** Popup size. */
    size: PropTypes.oneOf(_.without(UV5UI.SIZES, 'medium', 'big', 'massive')),
  ...
  setPopupStyle() {
    ...
    for (let i = 0; !this.isStyleInViewport(style) && i < positions.length; i++) { // eslint-disable-line no-plusplus
      style = this.computePopupStyle(positions[i]);
      position = positions[i];
    }

    // Append 'px' to every numerical values in the style
    style = _.mapValues(style, value => _.isNumber(value) ? value + 'px' : value); // eslint-disable-line prefer-template
    this.setState({
      style,
      position
    });
  ...
  computePopupStyle(positions) {
    ...
    if (_.includes(positions, 'top')) {
      ...
      style.top = Math.round(this.coords.bottom + pageYOffset - yOffset); // eslint-disable-line no-mixed-operators
      style.bottom = 'auto';
  ...
  hideOnScroll = (e) => { // eslint-disable-line no-unused-vars-rest/no-unused-vars
    this.setState({
      closed: true
    });
