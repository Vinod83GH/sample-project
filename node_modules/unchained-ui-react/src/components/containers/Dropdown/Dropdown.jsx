...
export default class Dropdown extends Component {
  static propTypes = {
    ...
    defaultOpen: PropTypes.bool, // eslint-disable-line react/no-unused-prop-types

    /** Currently selected label in multi-select. */
    defaultSelectedLabel: customPropTypes.every([ // eslint-disable-line react/no-unused-prop-types
    ...
    defaultValue: PropTypes.oneOfType([ // eslint-disable-line react/no-unused-prop-types
      PropTypes.number,
      PropTypes.string,
    ...
    /** Currently selected label in multi-select. */
    selectedLabel: customPropTypes.every([ // eslint-disable-line react/no-unused-prop-types
      customPropTypes.demand(['multiple']),
      PropTypes.oneOfType([
        PropTypes.string,
        PropTypes.number,
  ...
  getDropdownAriaOptions = (ElementType) => { // eslint-disable-line no-unused-vars-rest/no-unused-vars
    const {
      loading,
      disabled,
      search,
  ...
  moveSelectionBy = (offset, startIndex = this.state.selectedIndex) => {
    ...
    if (options[nextIndex].disabled) return this.moveSelectionBy(offset, nextIndex); // eslint-disable-line consistent-return
  ...
  scrollSelectedItemIntoView = () => {
    ...
    if (isOutOfUpperView) {
      ...
      menu.scrollTop = item.offsetTop + item.clientHeight - menu.clientHeight; // eslint-disable-line no-mixed-operators
    }
  }
  ...
  renderText = () => {
    ...
    if (searchQuery) {
      ...
      placeholderText = _.get(this.getSelectedItem(), 'text');
    } else if (hasValue) {
      placeholderText = _.get(this.getItemByValue(value), 'text');
    }

    return <div className={classes}>{placeholderText}</div>;
  }
  ...
  renderHiddenInput = () => {
    ...
    return (
      <select type={'hidden'} aria-hidden={'true'} name={name} value={value} multiple={multiple} role={name} aria-label={name}>
        <option value={''} />
  ...
  renderLabels = () => {
    ...
    return _.map(_.compact(selectedItems), (item, index) => { // eslint-disable-line consistent-return
      const defaultProps = {
  ...
  renderOptions = () => {
    ...
    if (noResultsMessage !== null && search && _.isEmpty(options)) {
      return <div className={'message'} aria-label={'no-results'}>{noResultsMessage}</div>;
    }
  ...
  renderMenu = () => {
    ...
    return (
      <DropdownMenu {...ariaOptions} className={menuClasses} aria-label={ariaLabel}>
