...
var isRenderMethod = function isRenderMethod(member) {
  return t.isClassMethod(member) && t.isIdentifier(member.key, { name: 'render' });
};

var hasRenderMethod = function hasRenderMethod(path) {
  return _lodash2.default.get(path, 'node.body.body').some(isRenderMethod);
};

var hasSuperClass = function hasSuperClass(_ref) {
  var superClass = _ref.node.superClass;
  return !!superClass;
};
